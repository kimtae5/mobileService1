...
public class MyInterceptor implements HandlerInterceptor {
	...
	//true를 리턴하면 Controller의요청 처리 메서드를 호출하고
	//false를 리턴하면 Controller의 메서드를 호출하지 않음
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
		...
		pw.println(request.getRemoteAddr() + ":" + request.getMethod() + ":" + request.getRequestURI());
		pw.flush();//버퍼의 내용을 전송
		pw.close();
		return true;
	}
	
	//Controller가 정상적으로 요청을 처리한 후 호출되는 메서드
